...
const FoodChart = () => {
  ...
  useEffect(() => {
    const fetchFoodData = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:9000/Food-charts?email=${userEmail}`);
        if (response.data && response.data.length > 0) {
          setFoodData(response.data);
        } else {
          setError('No food data found for this user.');
        }
      } catch (error) {
  ...
  const filterData = (data, timeframe) => {
    ...
    filtered.forEach(item => {
      const dateStr = new Date(item.consumedAt);
      let key;
      switch (timeframe) {
        case 'daily':
      ...
      aggregatedData[key] += item.caloriesIntake;
    });

    return { aggregatedData, filteredData: filtered };
  };

  const { aggregatedData, filteredData } = filterData(foodData, filter);
  const chartLabels = Object.keys(aggregatedData);
  const chartValues = Object.values(aggregatedData);
  const totalCaloriesIntake = chartValues.reduce((total, calories) => total + calories, 0);
  ...
  const options = {
    ...
    scales: {
      x: {
        title: {
          display: true,
          text: filter === 'daily' ? 'Time of Consumption' : 'Date',
        },
        grid: {
  ...
  return (
    <div className="chart-container">
      <h3 className="total-calories">Total Calories Taken: {totalCaloriesIntake} kcal</h3>
      ...
      <div className="food-log">
        ...
        <div className="log-list">
          {filteredData.map((entry) => (
            <div key={entry.consumedAt} className="log-entry">
              <span className="log-date">{new Date(entry.consumedAt).toLocaleString()}</span> - 
              <span className="log-details">{entry.foodName} - {entry.caloriesIntake} kcal</span>
            </div>
          ))}
        </div>
      </div>
